  ...
  */
  ...
  (function () {
       ...
       */
      BMapLib.MarkerManager = function (map,type,opts) {
              this._opts = opts || {};
              this._map = map;
              this._type=type;
       ...
       /**
         ...
         * mgr.getMarkerCount(15);
         ...
         MarkerManager.prototype.getMarkerCount = function (zoom) {
           ...
           MarkerManager.prototype.getMarkers = function (zoom) {
             var len = this._numMarkers.length,
                 t = this._numMarkers,
                count = 0;
             var visiblemarkers=[];
             for (var i = len; i--;) {
                 if(t[i].bInBounds&&(t[i].minZoom <= zoom && t[i].maxZoom >= zoom))
                 {
                  visiblemarkers.push(t[i]);
                 }
            }

      //如果隐藏掉所有marker则marker数量为0
             return this._visible ? visiblemarkers : [];
         }
         /**
            ...
            for (var i = num; i--;) {
               //在可视区域内 && 当前zoom符合marker的显示条件
                if (curBounds.containsPoint(t[i].point) && curZoom >= t[i].minZoom && curZoom <= t[i].maxZoom) {
                    ...
                    if (t[i].getLabel()&&t[i].type!="plane") {
                        if(curZoom>=9)
                        {
                          t[i].getLabel().show();
                        }
